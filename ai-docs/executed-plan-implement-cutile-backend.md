...
- **180 tests pass** (`cargo test --workspace`)
...
4. **Linux testing** — All development/testing was on macOS. Need `ssh tn` (RTX 3090) to validate real CUDA path.

## Next Steps

1. Write `#[cutile::module]` tile kernels (start with gemm, then rms_norm, softmax)
2. Test on RTX 3090 via `ssh tn`
3. Implement GPU-resident cache for KV and SSM state
4. Profile and tune tile sizes for sm_86
